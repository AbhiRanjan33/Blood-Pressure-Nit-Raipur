...
# # 4. Configure the Gemini API and Model (FIXED)
# genai.configure(api_key=GEMINI_API_KEY)
# model = genai.GenerativeModel(
#     "gemini-2.0-flash",
#     generation_config=generation_config,
#     system_instruction=system_prompt  # <-- The prompt is now passed here, at initialization
# )


# # --- Flask App Setup ---
# app = Flask(__name__)
# # Enable CORS (Cross-Origin Resource Sharing) to allow our HTML page to call this server
# CORS(app)

# # --- "AI" KNOWLEDGE BASE (Moved from JS to Python) ---
# COMMON_BP_DRUGS = [
...
#     # Search for exact match on generic OR brand name
...
#     # The system_prompt is now part of the model's initialization
    
#     # This is the user prompt with the specific data.
#     user_prompt = f"""
#     PATIENT PROFILE:
...
#         try:
#             # Call the LLM (FIXED: simplified call)
#             # The model instance already has the system prompt and generation config.
...
#             if "429" in str(e) or "503" in str(e): # 429: Too Many Requests, 503: Service Unavailable
...
#     # If all retries fail
#     return [{"type": "🔴 ERROR", "finding": "Could not analyze drug after multiple retries. See server console."}]
...
#     """
#     try:
#         # 1. Get patient profile from the frontend
#         profile = request.json
#         if not profile:
#             return jsonify({"error": "No patient profile provided"}), 400
            
#         # Create a single text block for the LLM
#         profile_text = f"""
...
#         # 2. Loop through each drug, fetch data, and analyze
#         for drug_def in COMMON_BP_DRUGS:
#             drug_name = drug_def["name"]
            
#             # 3. Fetch data from openFDA (server-side)
...
#                 "drugClass": drug_def["class"],
#                 "alerts": alerts,
#                 "fullData": drug_data # Send full data back to frontend
#             }
#             all_reports.append(report)

#         # 5. Send the complete list of reports back to the frontend
